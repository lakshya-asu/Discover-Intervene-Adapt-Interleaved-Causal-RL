...
class SkillScriptedOption(OptionPolicy):
    ...
    def __init__(self, subgoal: Subgoal, cfg: OptionConfig, var_name: str,
        ...
        self._step      = 0
        self._directive: List[int] = []   # VLM-supplied prefix; consumed first

    def set_directive(self, actions: List[int]):
        """
        Prepend a VLM-supplied action sequence that is executed before the
        normal macro.  Called by the transfer script after a VLM analysis.

        The directive is consumed once (not looped) then the macro takes over.
        """
        self._directive = list(actions)

    def act(self, obs) -> int:
        # If VLM directive is available, consume it first (no noise injection)
        if self._directive:
    ...
    def run(self, env, evgs: EVGS) -> Dict[str, Any]:
        obs     = env.get_obs() if hasattr(env, "get_obs") else {}
        self._step = 0
        # NOTE: do NOT clear _directive here; it is set externally before run()
        steps   = 0
        success = False
        trajectory: list = []
